...
import {
  Alert, Box, Button, Card, CardContent, Divider, IconButton, LinearProgress,
...
import { duracionTexto, hoyISO, minutosTrabajados } from '../dominio/horas.js';
import { COMPLETITUD, JORNADA_MINUTOS, completitudDe, notaSobreHoras } from '../dominio/registros.js';

/** Seccion principal: el reloj y dos tarjetas que se marcan con un clic. */
export default function SeccionMarcar({ datos, acciones, enviando, alEditar }) {
  const hoy = hoyISO();
  const enviadoHoy = datos.enviados[hoy] || null;
  const registro = enviadoHoy || datos.jornada || null;
  const estado = enviadoHoy ? 'enviado' : completitudDe(registro);
  ...
  return (
    <Stack spacing={2.5}>
      <Card variant="outlined">
        ...
        <Divider />
        <CardContent>
          {estado === COMPLETITUD.VACIO && !enviado && (
          ...
          {estado === COMPLETITUD.COMPLETO && !enviado && (
            <Stack spacing={2}>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                {observacionSucia && (
                  <Button onClick={() => acciones.editarJornada({ observacion })}>Guardar observacion</Button>
                )}
                <Box sx={{ flex: 1 }} />
                <Button
                  variant="contained"
                  ...
                  disabled={enviando}
                  onClick={async () => {
                    if (observacionSucia) await acciones.editarJornada({ observacion });
                    acciones.enviar([{ ...registro, observacion }]);
                  }}
                >
                  Enviar mi jornada
                </Button>
