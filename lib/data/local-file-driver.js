...
const emptyState = () => ({ users: {}, submissions: {}, drafts: {}, activity: {}, nextActivityId: 1 });
const emptyDrafts = () => ({ today: null, bulk: null });
...
export function createLocalFileDriver() {
  return {
    ...
    async getDrafts(userId) {
      return { ...emptyDrafts(), ...bucket(load().drafts, userId, emptyDrafts()) };
    },

    async saveDraft(userId, kind, payload) {
      const store = load();
      bucket(store.drafts, userId, emptyDrafts())[kind] = payload;
      persist();
    },

    async deleteDraft(userId, kind) {
      const store = load();
      bucket(store.drafts, userId, emptyDrafts())[kind] = null;
      persist();
    },
