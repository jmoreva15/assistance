...
import { readDrafts, writeDrafts } from './draft-storage.js';
import { clearUserId, readUserId, touchSession, writeUserId } from './session-storage.js';
...
export function useAttendance() {
  ...
  useEffect(() => {
    ...
    const attempt = async (index) => {
      try {
        const next = await apiClient.resumeSession(stored);
        if (cancelled) return;
        setWorkspace(next);
        touchSession();
        setLoading(false);
        ...
        const delay = RETRY_DELAYS[Math.min(index, RETRY_DELAYS.length - 1)];
  ...
  const restampToday = useCallback(
    (field) => {
      if (todaySubmission) return setError('Hoy ya fue enviado.');
      const time = currentTime();
      const current = drafts.today ?? createRecord({ date: today });
      const record = { ...current, [field]: time };
      const name = field === 'clockIn' ? 'entrada' : 'salida';
      saveDrafts({ ...drafts, today: record }, ACTIONS.DAY_EDITED, `${name} vuelta a marcar a las ${time}`);
      setNotice(`${name === 'entrada' ? 'Entrada' : 'Salida'} vuelta a marcar a las ${time}.`);
      return { ok: true };
    },
    [drafts, saveDrafts, today, todaySubmission],
  );
