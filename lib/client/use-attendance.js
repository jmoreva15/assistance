...
const EMPTY = { user: null, submissions: [], drafts: { today: null, bulk: null }, activity: [] };
...
export function useAttendance() {
  ...
  const dropSession = useCallback((message) => {
    clearUserId();
    setWorkspace(EMPTY);
    setError(message);
  }, []);
  ...
  const run = useCallback(
    async (work, message) => {
      ...
      try {
        ...
        if (failure.code === 'SESSION_GONE') dropSession(failure.message);
