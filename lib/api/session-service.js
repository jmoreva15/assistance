...
export async function loadWorkspace(user) {
  const db = repository();
  const [rawDrafts, submissions, activity] = await Promise.all([
    db.getDrafts(user.id),
    db.listSubmissions(user.id),
    db.listActivity(user.id),
  ]);
  const drafts = await discardStaleToday(user.id, rawDrafts);
